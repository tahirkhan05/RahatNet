...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, data: null, error: { code: 'VALIDATION_ERROR' as const, message: 'Invalid JSON body.', statusCode: 400 }, requestId },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    await adminFirestore.collection(COLLECTIONS.RAW_REPORTS).doc(reportId).set({
      reporterId: uid,
      type: data.type,
    ...
    // 5. Best-effort RTDB entry to wake up the AI Cloud Function.
    try {
