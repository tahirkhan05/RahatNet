...
import { TrendingUp, TrendingDown, Minus, Users, AlertTriangle, CheckCircle2, Activity } from 'lucide-react';
...
function StatCard({
  label, value, icon, iconColor, trend, trendLabel, isLoading, highlight,
}: StatCardProps) {
  return (
    <div
      className={[
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          {isLoading ? (
            <div className="mt-1 h-8 w-16 skeleton-shimmer rounded" />
          ) : (
            <p className={[
      ...
      {!isLoading && trend != null && trendLabel != null && (
        <div className="mt-3 flex items-center gap-1 text-xs">
          {trend === 'up' ? (
            <TrendingUp className="h-3 w-3 text-destructive" aria-hidden="true" />
          ) : trend === 'down' ? (
...
export function ImpactMetrics({ stats, isLoading }: ImpactMetricsProps) {
  ...
  const totalTrend =
    !baseline ? 'neutral' :
    stats.total > baseline.total ? 'up' :
    stats.total < baseline.total ? 'down' : 'neutral';

  const criticalTrend =
    !baseline ? 'neutral' :
    stats.critical > baseline.critical ? 'up' :
    stats.critical < baseline.critical ? 'down' : 'neutral';
  ...
  return (
    <div className="space-y-3" aria-label="Impact metrics">
      <StatCard
        ...
        trendLabel={
          !baseline ? undefined :
          stats.total === baseline.total ? 'No change' :
      ...
      <StatCard
        ...
        trendLabel={
          !baseline ? undefined :
          stats.critical === baseline.critical ? 'Stable' :
